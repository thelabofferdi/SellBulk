...
impl DecisionEngine {
    ...
    pub fn decide(&self, context: DecisionContext) -> Result<Action> {
        ...
        // Rule 2: If quotas exceeded -> Delay
        if !context.quotas_available {
            return Ok(Action::Delay { seconds: 3600 });
        }

        // Rule 3: If anger/threat detected -> Alert human
